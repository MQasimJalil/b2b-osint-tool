...
CRAWLED_DOMAINS_FILE = CACHE_DIR / "crawled_domains.jsonl"
HOMEPAGE_FEATURES_DIR = CACHE_DIR / "homepage_features"
DEDUP_RESULTS_FILE = CACHE_DIR / "dedup_results.jsonl"

# Create directories
CACHE_DIR.mkdir(exist_ok=True)
HOMEPAGE_FEATURES_DIR.mkdir(exist_ok=True)


# ============================================================================
# Crawled Domains Tracker
# ============================================================================

def track_crawled_domain(domain: str):
    """
    Add domain to crawled domains list.
    ...
    with open(CRAWLED_DOMAINS_FILE, 'a', encoding='utf-8') as f:
        f.write(json.dumps(entry) + "\n")


def get_crawled_domains() -> List[str]:
    """
    Get list of all previously crawled domains.
    """
    if not CRAWLED_DOMAINS_FILE.exists():
        return []

    domains = []
    with open(CRAWLED_DOMAINS_FILE, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
                try:
...
def save_homepage_features(domain: str, features: Dict):
    """Cache homepage features for future comparisons."""
    cache_file = HOMEPAGE_FEATURES_DIR / f"{domain}.json"
...
def load_homepage_features(domain: str) -> Optional[Dict]:
    """Load cached homepage features."""
    cache_file = HOMEPAGE_FEATURES_DIR / f"{domain}.json"
...
def check_before_crawl(
    ...
    """
    # Get already crawled domains
    already_crawled = get_crawled_domains()
    ...
    for candidate_domain, pattern_score in pattern_matches.items():
        # Load cached features for candidate
        candidate_features = load_homepage_features(candidate_domain)

        if not candidate_features:
            # If features not cached, fetch and extract (shouldn't happen normally)
            ...
            if candidate_html:
                ...
                save_homepage_features(candidate_domain, candidate_features)
            else:
                continue
    ...
    if best_total_score >= duplicate_threshold:
        ...
        # Log to dedup results
        with open(DEDUP_RESULTS_FILE, 'a', encoding='utf-8') as f:
        ...
        # Cache homepage features for future comparisons
        save_homepage_features(domain, new_features)
