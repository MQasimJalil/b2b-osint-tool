...
class ContactEnricher:
    ...
    def update_profile_with_enrichment(
        ...
        """
        Update company profile.json with enrichment results.
        ...
        try:
            ...
            if not dry_run:
                ...
                logger.info(f"[{enrichment.domain}] Updated profile with enrichment data")

            return True

        except Exception as e:
            logger.error(f"[{enrichment.domain}] Failed to update profile: {e}")
            return False
...
def enrich_company(domain: str, profile: Dict = None) -> EnrichmentResult:
    ...
    if profile:
        ...
        if not profile_path.exists():
            raise FileNotFoundError(f"Profile not found for {domain}")
