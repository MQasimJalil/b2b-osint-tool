...
from app.db.repositories.discovery_repo import (
    add_discovered_domain_sync,
    get_discovered_domains_set_sync,
    save_query_cache_sync,
    get_completed_queries_sync,
    save_vetting_result_sync,
    get_vetting_result_sync
)
from app.db.mongodb_session import init_db, close_db
import asyncio

# Import unified query generator
from .query_generator import generate_queries, QueryGeneratorConfig, load_config
...
BLACKLIST = [
    ...
    'exporthub.', 'etradeasia.', 'alibaba.', 'etsy.', 'walmart.', 'tiktok.', 'temu.', 'daraz.', "reddit."
]
...
def _ensure_mongodb():
    ...
    if not _mongodb_initialized:
        try:
            ...
            print("✓ MongoDB initialized for discovery service")
        except Exception as e:
            print(f"Warning: Failed to initialize MongoDB: {e}")
            print("Discovery service will continue but data won't be persisted to MongoDB")


def _load_discovered_set() -> Set[str]:
    """Load discovered domains from MongoDB"""
    try:
        return get_discovered_domains_set_sync()
    except Exception as e:
        print(f"Warning: Failed to load discovered domains from MongoDB: {e}")
        return set()


def _load_completed_queries() -> Set[str]:
    """Load completed queries from MongoDB (engine::query format)"""
    try:
        return get_completed_queries_sync()
    except Exception as e:
        print(f"Warning: Failed to load completed queries from MongoDB: {e}")
        return set()


def _generate_queries_sync(industry: str, regions: List[str], languages: List[str]) -> List[str]:
    """
    Generate queries using the unified query generator.
    This is a synchronous wrapper for the async generate_queries function.
    """
    # Load config for settings
    cfg = load_config()
    limits = cfg.get("limits", {})

    # Create query generator config
    query_config = QueryGeneratorConfig(
        use_ai_variants=True,
        max_queries=limits.get("max_queries", 400),
        per_family_cap=limits.get("per_family_cap", 50),
        regions=regions,
    )

    # Run the async query generation in sync context
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        queries, _ = loop.run_until_complete(generate_queries([industry], query_config))
        return queries
    finally:
        loop.close()
...
def _soft_vet(url: str, timeout: float) -> Dict[str, bool]:
    """Soft vetting check with MongoDB caching"""
    key = _normalize_domain(url)

    # Check MongoDB cache first
    try:
        cached = get_vetting_result_sync(key)
        if cached:
            return cached
    except Exception as e:
        print(f"Warning: Failed to check vetting cache for {key}: {e}")
    ...
    try:
        with httpx.Client(timeout=timeout, follow_redirects=True, headers={"User-Agent": random.choice(HEADERS_LIST)}) as client:
            ...
            try:
                ...
                print(f"Warning: Failed to save vetting result for {key}: {e}")
...
def _save_query_cache(query: str, engine: str, domains: List[str]):
    """Save completed query to MongoDB cache"""
    try:
        save_query_cache_sync(engine, query, domains)
    except Exception as e:
        print(f"Warning: Failed to save query cache for {engine}::{query}: {e}")
...
def discover_domains(industry: str, max_results: int = 500) -> List[str]:
    ...
    queries = _generate_queries_sync(industry, regions, languages)
                        ...
                        for d in page_domains:
                            ...
                            with file_lock:
                                ...
                                try:
                                    ...
                                    print(f"Warning: Failed to save discovered domain {d}: {e}")
